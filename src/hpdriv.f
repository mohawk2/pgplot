...
C
   ...
   91     IF (CHR(LCHR:LCHR).EQ.' ') THEN
          ...
          RBUF(2) = 1
      END IF
C grps02 borrowed from the PS driver, which is quite similar
      CALL GRPS02(UNIT,EC//'E')
      PEON = .FALSE.
      NREL = 0
      RETURN
C
...
C Enter HPGL, black pen, thick lines have rounded ends and joins
  110 CONTINUE
      CALL GRPS02(UNIT,EC//'%0BINSP1LA1,4,2,4PA1,1')
      LASTI = 1
      LASTJ = 1
      RETURN
C
...
C simple pen-down move
      IF (I0.EQ.LASTI .AND. J0.EQ.LASTJ) THEN
          ...
          CALL GRHPEC((I1-I0), (J1-J0), INSTR, L)

C pen up to start of line, then pen down to end of line
      ELSE
          CALL GRHPEC((I0-LASTI), (J0-LASTJ), DUMMY1, L1)
          CALL GRHPEC((I1-I0), (J1-J0), DUMMY2, L2)
...
C pen up move to position, then pen down
      CALL GRHPEC((I1-LASTI), (J1-LASTJ), DUMMY1, L1) 
      CALL GRHPEC(0, 0, DUMMY2, L2)
...
C Just another point
          ELSE
              CALL GRHPEC((I0-LASTI),(J0-LASTJ),INSTR,L)
              LASTI = I0
              LASTJ = J0
          END IF
...
C*GRHPEC -- PGPLOT HPGL2 driver, convert integers to coded ascii
C+
      SUBROUTINE GRHPEC(X,Y,OUTSTR,L)
C
