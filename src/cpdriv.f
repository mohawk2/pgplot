C*CPDRIV -- PGPLOT Color PostScript driver (landscape mode)
C+
      SUBROUTINE CPDRIV (IFUNC, RBUF, NBUF, CHR, LCHR)
...
C-----------------------------------------------------------------------
      ...
      PARAMETER (DEFNAM='pgplot.cp')
C -- parameters for landscape mode
      PARAMETER (TYPE='CPS')
      PARAMETER (WIDTH=8599, HEIGHT=7799)
      PARAMETER (SETUP='8150 1450 translate 90 rotate')
C -- parameters for portrait mode
C     PARAMETER (TYPE='VCPS')
C     PARAMETER (WIDTH=7799, HEIGHT=8599)
C     PARAMETER (SETUP='350 1450 translate')
C --
C
C
      INTEGER  LASTI, LASTJ, LOBUF, NPAGE, NPTS, UNIT
      SAVE     LASTI, LASTJ, LOBUF, NPAGE, NPTS, UNIT
      INTEGER  IER, I0, J0, I1, J1, L
      INTEGER  CI, LW
      INTEGER  GROPTX
      LOGICAL  START
      SAVE     START
...
C
  900 WRITE (MSG,
     1  '(''Unimplemented function in CP device driver: '',I10)') IFUNC
      CALL GRWARN(MSG)
      NBUF = -1
      RETURN
C-----------------------------------------------------------------------
