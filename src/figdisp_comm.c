...
/*			away. */
/* 28-Apr-1993  KS/AAO  Have kluged this version so it works with our version */
/*                      of Figdisp, which doesn't expect data in network byte */
/*                      order. Eventually we will sort out the versions of */
/*                      Figdisp and Pgplot. */
/* 13-Oct-1993  KS/AAO  Changed the fudge factor in figdisp_maxbuflen from 10 */
/*                      to 100, as suggested by SNS. */
/* 24-Aug-1998  KS/AAO  Added netinet/in.h. This is required by HP-UX. */
...
int *len;		/* the number of shorts in the response */
{
	...
	if (xerror || !fd_dispopen)
	{
		*len = 0;
		retval = NULL;
	} else figdisp_convbufin(retval, *len);
	return(retval);
}
...
int dev;	/* The figdisp display to open */
{
	...
	if (!fd_dispopen)
	{
		...
		if ((selatom=XInternAtom(display,&lockname[0],True)) == None)
		{
		...
		if ((incratom=XInternAtom(display,&lockname[0],True)) == None)
		{
...
int figdisp_maxbuflen()
{
	...
	retval= (XMaxRequestSize(tmp) - 100)*2; /* SNS had 10, not 100 (KS) */
...
u_short *buf;	/* The command buffer to convert */
int len;		/* The number of shorts in the command buffer. */
{
	u_short testshort=0x1234;
	u_short realcom;
