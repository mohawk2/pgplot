...
C-----------------------------------------------------------------------
      ...
      INTEGER  II,III,JJ,JJJ,ISV
      INTEGER  OCI, CI
      REAL     PPA(6),PINV(6), WGT, XY0, YY0, DI, DJ

      REAL     BUFFER(260), OCT(3,256), CBUF(4), ATMP, BTMP
      CHARACTER*1 CHR
...
C Use that pixel as the basis for the range of pixels that need to
C be considered.
C
      DO 120 IY=IY1,IY2
...
C Apply a linear rolloff (factor-of-2 comes out in the wash)
                  IF ((DX.LT.1) .AND. (DY.LT.1)) THEN
                     ATMP = (1 - DX) * (1-DY)
                     WGT = WGT + ATMP
                     CBUF(2) = CBUF(2) + ATMP * (A(III,JJJ,1) - A1)/ARANGE
                     CBUF(3) = CBUF(3) + ATMP * (A(III,JJJ,2) - A1)/ARANGE
                     CBUF(4) = CBUF(4) + ATMP * (A(III,JJJ,3) - A1)/ARANGE
                  ENDIF
