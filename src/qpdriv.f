      SUBROUTINE    QPDRIV (IFUNC, RBUF, NBUF, CHR, LCHR)
...
C Version 2.1  - 1988 Oct 10 - [PPM] John Benson's changes; upside down
C Version 2.1a - 1991 Jun 28 - [TJP] Standardization.
C Version 2.2  - 1991 Nov  6 - [TJP] Standardization.
C-----------------------------------------------------------------------
C The GRQM00 subroutine is physically located in the QMDRIVER.FOR file.
C-----------------------------------------------------------------------
C
C Supported device: Any QMS or Talaris printer that accepts the QUIC 
C                   page description language.  4-bit mode is used.
C
C Device type code: /VQMS (portrait mode; QMDRIVER or /QMS landscape)
C
C Default file name: PGPLOT.QPPLOT.
C
C Default view surface dimensions: 7.75 inches vertical x 10.25 inches 
C                                  horizontal (portrait mode), margins
C                                  0.5 inches on top and left of page.
C                                  (QMS printers don't print anything on
C                                   the leftmost 0.3" or so of the page)
C
...
C
C Obtaining hardcopy:  $ PRINT/QUEUE=QMS file.type (site-dependent)
C-----------------------------------------------------------------------
CE
      CHARACTER*(*) DEVTYP, DEFNAM
      PARAMETER    (DEFNAM='PGPLOT.QPPLOT')
      PARAMETER    (DEVTYP='VQMS')
C
...
C
  900 WRITE (MSG,'(I10)') IFUNC
      CALL GRWARN('Unimplemented function in '//DEVTYP//
     :            ' device driver:'//MSG)
      NBUF = -1
      RETURN
C
...
C
   90 CONTINUE
C***  CALL GRWARN ('VQMS open workstation')
C
C     -- Get a Unit number.  
C
      CALL GRGLUN(UNIT)
...
C
      ...
      IF (IER.NE.0) THEN
          TEMP = CHR(1:LCHR)
          CALL GRWARN('Cannot open output file for '//DEVTYP//
     :                ' plot: '//TEMP)
          RBUF(2) = 0
          CALL GRFLUN(UNIT)
          RETURN
...
C     -- Set portrait mode, set margins, enter vector graphics mode
C
      BUFLEN = 1
...
C
  100 CONTINUE
C***  CALL GRWARN ('VQMS Close Workstation')
      IF (NOTHIN) THEN
C
...
C
  110 CONTINUE
C***  CALL GRWARN ('VQMS Begin Picture')
      ENDFIL = .FALSE.
...
C
  120 CONTINUE
      ...
      IF (IC.EQ.0) RETURN
      I0 = NINT(RBUF(1) * QXSCAL)
      J0 = NINT((QYSIZE - RBUF(2)) * QYSCAL)
      I1 = NINT(RBUF(3) * QXSCAL)
      J1 = NINT((QYSIZE - RBUF(4)) * QYSCAL)
  125 CONTINUE
...
C
  140 CONTINUE
C***  CALL GRWARN ('VQMS End Picture')
      ENDFIL = .TRUE.
      IF (.NOT. NOTHIN) THEN
