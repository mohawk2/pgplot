      SUBROUTINE    QMDRIV (IFUNC, RBUF, NBUF, CHR, LCHR)
...
C
   90 CONTINUE
C***  CALL GRWARN ('QMS open workstation')
C
C     -- Get a Unit number.  VMS specific!  Other systems (e.g. UNIX) 
C     -- should have a similar call to return a usable I/O unit number.
C
      CALL GRGLUN(UNIT)
C
C     -- Open the file.
...
C     -- Set landscape mode, set margins, enter vector graphics mode
C
      BUFLEN = 1
...
C
  100 CONTINUE
C***  CALL GRWARN ('QMS Close Workstation')
      IF (NOTHIN) THEN
C
...
C
      ...
      ENDIF
C
C     -- Yet another VMS-specific call.  Return UNIT to free pool.
C
      CALL GRFLUN(UNIT)
      RETURN
C
...
C
  110 CONTINUE
C***  CALL GRWARN ('QMS Begin Picture')
      ENDFIL = .FALSE.
