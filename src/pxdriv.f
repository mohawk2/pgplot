...
C
C Default device name: PGPLOT.PXPLOT.
C
...
C     -- dimensions of plot buffer
     ...
     2      FORM='UNFORMATTED', RECL=1, IOSTAT=IER)
      IF (IER.NE.0) THEN
...
C
  110 CONTINUE
      NPICT = NPICT+1
      IF (NPICT.GT.1) CALL FPUTC(UNIT, CHAR(FF))
      CALL GRIBF1(BX*BY, %val(BITMAP), 'C0'X)
      RETURN
C
