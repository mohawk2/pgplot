...
C Version 1.4 - 1989 Mar 14 - Cray cf77 FORTRAN under UNICOS.
...
C      INTEGER LIB$GET_VM, LIB$FREE_VM
      ...
      INTEGER TKBUF(100),TKTERM(6), BUFFER(BUFSIZ)
      SAVE TKTERM, BUFFER
C     BYTE    TKBUF(100), TKTERM(6)
...
C        (code US)
      CALL GRTE02(TKTERM,6,BUFFER,BUFLEV,UNIT)
C     -- flush buffer
      CALL GRWTERM(UNIT, BUFFER, BUFLEV)
      LASTI = -1
      RETURN
