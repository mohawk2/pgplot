...
#else
#define		USLEEP	/* Use the usleep(3) call to wait between existance */
			/* checks.  This is nice on the CPU and gives decent */
			/* response time.  If usleep(3) is available on your */
			/* system it is definately the best choise, assuming */
			/* a wise choice of sleeptime values by the user. */
			/* A minimum sleeptime value can be set in figdisp.h. */
#endif
