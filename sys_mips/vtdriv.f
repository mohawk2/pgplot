...
C      PARAMETER (CURSOR=CHAR(27)//'[24;1f')
C      PARAMETER (VTERAS=CHAR(27)//'[2J'//CHAR(27)//'[H')
C      PARAMETER (VTHOME=CHAR(27)//'[H')
      PARAMETER (CURSOR='[24;1f')
      PARAMETER (VTERAS='[2J[H')
      PARAMETER (VTHOME='[H')
      INTEGER BUFSIZ
...
C
      ...
      INTEGER  MONO, IR, IG, IB, ICH, ICX, ICY
      INTEGER  VTCODE(0:3)
      INTEGER  GROPTX
      LOGICAL  APPEND
      REAL     CH, CL, CS
...
C          CALL GRWARN('Cannot open output file for '//TYPE//' plot: '//
C     1                CHR(:LCHR))
          RBUF(2) = 0
          CALL GRFLUN(UNIT)
      ELSE
...
C     -- position graphics cursor
      WRITE (UNIT,111) CHAR(27),ICX,ICY
  111 FORMAT(A,'PpP[', I4 ,',', I4 ,']')
      ...
      END IF
C     -- back to text mode
      WRITE (UNIT,'(2A1)') CHAR(27),CHAR(92)
      RBUF(1) = ICX
      RBUF(2) = ICY
      CHR = CHAR(ICH)
      LASTI = -1
