C*VXDRIV -- PGPLOT Talaris/EXCL driver (portrait mode)
C+
      SUBROUTINE    VXDRIV (IFUNC, RBUF, NBUF, CHR, LCHR)
...
C
C Device type code: /VEXCL
C
C Default file name: PGPLOT.VXPLOT.
C
...
C
      ...
      REAL          EXSIZE, EYSIZE
      INTEGER       PGO, XOFF, YOFF
C--
C     PARAMETER    (DEFNAM = 'PGPLOT.EXPLOT')
C     PARAMETER    (DEVTYP = 'EXCL')
C     PARAMETER    (EXSIZE = 10250.0)
C     PARAMETER    (EYSIZE =  7750.0)
C     PARAMETER    (PGO = 1)
C     PARAMETER    (XOFF = (11000 - EXSIZE) / 2)
C     PARAMETER    (YOFF =  (8500 - EYSIZE) / 2)
C
      PARAMETER    (DEFNAM = 'PGPLOT.VXPLOT')
      PARAMETER    (DEVTYP = 'VEXCL')
      PARAMETER    (EXSIZE =  7750.0)
      PARAMETER    (EYSIZE = 10250.0)
      PARAMETER    (PGO = 0)
      PARAMETER    (XOFF =  (8500 - EXSIZE) / 2)
      PARAMETER    (YOFF = (11000 - EYSIZE) / 2)
C--
...
C
      ...
      CHARACTER*80  INSTR
      CHARACTER*10  MSG
      INTEGER       UNIT, IER, I0, J0, I1, J1, IC
...
C
      NBUF = 2
      RBUF(1) = UNIT
      OPEN (UNIT=UNIT, FILE=CHR(1:LCHR), 
Cvms &      DEFAULTFILE=DEFNAM, CARRIAGECONTROL='LIST',
     &      STATUS='NEW', DISPOSE='DELETE',
     &      IOSTAT=IER)
      IF (IER.NE.0) THEN
...
C         origin to the top left of the physical page at XOFF,YOFF 
C         (TALORG).
C
      BUFFER = ESCLB//'3y'
     &       //ESCLB//'0p'
     &       //ESCLB//'0;;;0*c'
      WRITE (BUFFER (7:7), '(I1)') PGO
      WRITE (BUFFER (18:31), 1000) ESCLB, XOFF, YOFF
...
C2000 FORMAT (A2, ';', I5.5, ';', I4.4, A2) ! (landscape)
 2000 FORMAT (A2, ';', I4.4, ';', I5.5, A2) ! (portrait)
...
C3000    FORMAT (I5.5, ':', I4.4, ';') ! (landscape)
 3000    FORMAT (I4.4, ':', I5.5, ';') ! (portrait)
